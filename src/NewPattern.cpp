                                 ...
                                 int Index_CNT)
{
    ...
    // 기존 배열을 Eigen::Map으로 래핑
	Eigen::VectorXd ref_rl_pr = Eigen::Map<Eigen::VectorXd>(Ref_RL_PR, 6);
	Eigen::VectorXd ref_ll_pr = Eigen::Map<Eigen::VectorXd>(Ref_LL_PR, 6);
	Eigen::VectorXd rl_th = Eigen::Map<Eigen::VectorXd>(RL_th, 6);
	Eigen::VectorXd ll_th = Eigen::Map<Eigen::VectorXd>(LL_th, 6);

	// IK 계산
	Eigen::VectorXd rl_th_ik = BRP_RL_IK(ref_rl_pr, rl_th, L0, L3, L4, L6);
	Eigen::VectorXd ll_th_ik = BRP_LL_IK(ref_ll_pr, ll_th, L0, L3, L4, L6);

	// 결과를 RL_th, LL_th에 반영
	Eigen::VectorXd::Map(RL_th, 6) = rl_th_ik;
	Eigen::VectorXd::Map(LL_th, 6) = ll_th_ik;

}


void IK_Function::Change_Com_Height(double h) {Com_Height = h;}
