...
void Trajectory::Make_turn_trajectory(double angle)
{
    ...
    double T_rise = walktime_n * 0.3;  // 올라가는 구간 시간
    double T_fall = walktime_n * 0.3;  // 내려가는 구간 시간

    for (int i = 0; i < walktime_n; i++)
    {
        if (i < 0.1 * walktime_n) Turn_Trajectory(i) = 0;
        else if (i < 0.4 * walktime_n) {
            double t_rel = i - 0.1 * walktime_n;
            Turn_Trajectory(i) = Sinusoidal(t_rel, T_rise, angle); // 상승 궤적 (0 → angle)
        }
        else if (i < 0.6 * walktime_n) Turn_Trajectory(i) = angle; // remain 
        else if (i < 0.9 * walktime_n) {
            double t_rel = i - 0.6 * walktime_n;
            Turn_Trajectory(i) = Sinusoidal(t_rel, T_fall, angle);
            Turn_Trajectory(i) = angle - Turn_Trajectory(i);  // 반전, 하강 궤적 (angle → 0)
        }
        else Turn_Trajectory(i) = 0;
    }
}
