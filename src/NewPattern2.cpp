...
Trajectory::Trajectory()
{
	...
	A << 1, del_t, del_t * del_t / 2, // 시스템의 상태 : 위치 속도 가속도
		0, 1, del_t,
		0, 0, 1;
	B << del_t * del_t * del_t / 6, 
				del_t * del_t / 2, 
				del_t; // 입력 행렬 - 가속도를 제어 입력 취급
...
//     {
//         double time = i * del_t;
//         if (time < 2.5 * walktime) Footpos[i] = 0; // wait for lf
...
MatrixXd Trajectory::Huddle_Xcom()
{
	...
	for (int i = 0; i < sim_n + 19; i++)
	{
		...
		else if (i < 4.0 * float_walktime_n) zmp_ref[i] = step;
		else if (i < 5.5 * float_walktime_n) zmp_ref[i] = 1.1*step;
		else if (i < 6.1 * float_walktime_n) zmp_ref[i] = 1.2*step;
		else if (i < 7.5 * float_walktime_n) zmp_ref[i] = 1.2*step;
...
MatrixXd Trajectory::Huddle_Ycom()
{
	...
	for (int i = 0; i < sim_n + 19; i++)
	{
		...
		else if (i < 1.85 * float_walktime_n) zmp_ref[i] = -0.05;
		else if (i < 6.0 * float_walktime_n) zmp_ref[i] = 0.063;
		else if (i < 7.4 * float_walktime_n) zmp_ref[i] = -0.05; // minus means right way tiliting 
		else zmp_ref[i] = 0.00;


		// if (i < 0.75 * float_walktime_n) zmp_ref[i] = 0;
		// else if (i < 1.75 * float_walktime_n) zmp_ref[i] = -0.053;
...
MatrixXd Trajectory::RF_xsimulation_huddle()
{
	XStep = Equation_solver(0, 0.7*walktime, 0, 1.2 * step);
	step_n = 8;
	sim_n = walktime_n * step_n;
	double del_t = 1 / freq;
	RowVectorXd Footpos(sim_n);
	...
	{
		double time = i * del_t;
		if (time < 3.5 * walktime) Footpos[i] = 0;	// 0.0 ~ 3.5, RF waiting, 
		else if (time < 4.2 * walktime) Footpos[i] = Step(time - 3.5 * walktime); // 3.5 ~ 4.5, RF front moving
		else Footpos[i] = 1.2 * step; // 4.5~, Maintain the step. 
	};
	return Footpos;
};
...
MatrixXd Trajectory::RF_zsimulation_huddle(double h, double COM_h)
{
	XStep = Equation_solver(0, 0.5 * walktime, 0, h);
	XStride = Equation_solver(0, 0.2 * walktime, h, 0);
	step_n = 8;
	sim_n = walktime_n * step_n;
	double del_t = 1 / freq;
	...
	for (int i = 0; i < sim_n; i++)
	{
		...
		else if (time < 3.3 * walktime) Footpos[i] = 0.5*COM_h; // ~3.3, RF waiting 
		else if (time < 3.8 * walktime) Footpos[i] = 0.8*Step(time - 3.3 * walktime) + 0.5*COM_h; // 3.3~3.5, RF up
		else if (time < 4.4 * walktime) Footpos[i] = 0.8*h + 0.5*COM_h; // 3.5~4.5, keep RF up 
		else if (time < 4.6 * walktime) Footpos[i] = 0.8*Stride(time - 4.4 * walktime) + 0.5*COM_h; // 4.5~4.7, RF down
		else Footpos[i] = 0.5*COM_h;
	};
	return Footpos;
};
...
MatrixXd Trajectory::LF_xsimulation_huddle()
{
	...
	{
		double time = i * del_t;
		if (time < 1.0 * walktime) Footpos[i] = 0;
		else if (time < 1.5 * walktime) Footpos[i] = Step(time - 1.0 * walktime);	// 1.0 ~ 1.5,  1 step moving
		else if (time < 6.8 * walktime) Footpos[i] = step; // maintain the step
		else if (time < 7.1 * walktime) Footpos[i] = Stride(time - 6.8 * walktime) + step;
		else Footpos[i] = 1.2 * step;
	};
	return Footpos;
};
...
MatrixXd Trajectory::LF_zsimulation_huddle(double h, double COM_h)
{
	XStep = Equation_solver(0, 0.3 * walktime, 0, h);
	XStride = Equation_solver(0, 0.3 * walktime, h, 0);
	step_n = 8;
	sim_n = walktime_n * step_n;
	double del_t = 1 / freq;
	RowVectorXd Footpos(sim_n);
	for (int i = 0; i < sim_n; i++)
	{
		double time = i * del_t;
		if (time < 0.8 * walktime) Footpos[i] = 0.5*COM_h;
		else if (time < 1.1 * walktime) Footpos[i] = 0.75*Step(time - 0.8 * walktime) + 0.5*COM_h; // 0.8~1.1 LL up
		else if (time < 1.5 * walktime) Footpos[i] = 0.75*h + 0.5*COM_h; // 1.1~1.5 LL maintenance 
		else if (time < 1.8 * walktime) Footpos[i] = 0.75*Stride(time - 1.5 * walktime) + 0.5*COM_h; //	1.5~1.8 LL down to first state
		else if (time < 6.5 * walktime) Footpos[i] = 0.5*COM_h;		// 1.8 ~ 7.0 maintain first state (RL Moving)
		else if (time < 6.8 * walktime) Footpos[i] = 0.2 * Step(time - 6.5 * walktime) + 0.5*COM_h; // repositioning
		else if (time < 7.1 * walktime) Footpos[i] = 0.2 * h + 0.5*COM_h;
		else if (time < 7.4 * walktime) Footpos[i] = 0.2 * Stride(time - 7.1 * walktime) + 0.5*COM_h;
		else Footpos[i] = 0.5*COM_h;
