...
MatrixXd Trajectory::RF_zsimulation_sitdown3(double h)
{
	...
	for (int i = 0; i < 500; i++)
	{
		if (i < 100) Footpos[i] = 0;
		else if (i < 400) Footpos[i] = Step(i - 100);
		else if (i < 500) Footpos[i] = h;
		else Footpos[i] = h;
	};
	return Footpos;
}
...
MatrixXd Trajectory::RF_zsimulation_standup3(double h)
{
	...
	for (int i = 0; i < 500; i++)
	{
		if (i < 100) Footpos[i] = h;
		else if (i < 400) Footpos[i] = Step(i - 100);
		else if (i < 500) Footpos[i] = 0;
		else Footpos[i] = 0;
	};
	return Footpos;
}
...
MatrixXd Trajectory::RF_ysimulation_leftwalk()
{
	...
	{
		double time = i * del_t;
		if (time < 1.1 * walktime) Footpos[i] = -0.06;
		else if (time < 1.4 * walktime) Footpos[i] = -0.06;
		else if (time < 2.1 * walktime) Footpos[i] = -0.06;
		else if (time < 2.4 * walktime) Footpos[i] = Step(time - 2.1 * walktime) - 0.06;
		else if (time < 3.1 * walktime) Footpos[i] = -0.06 + step;
		else if (time < 3.4 * walktime) Footpos[i] = Step(time - 3.1 * walktime) - 0.06 + step;
		else Footpos[i] = -0.06 + 2 * step;
	};
	return Footpos;
}
...
MatrixXd Trajectory::LF_ysimulation_leftwalk()
{
	...
	for (int i = 0; i < sim_n; i++)
	{
		...
		if (time < 1.6 * walktime) Footpos[i] = 0.06;
		else if (time < 1.9 * walktime) Footpos[i] = Step(time - 1.6 * walktime) + 0.06;
		else if (time < 2.6 * walktime) Footpos[i] = 0.06 + step;
		else if (time < 2.9 * walktime) Footpos[i] = Step(time - 2.6 * walktime) + 0.06 + step;
		else if (time < 3.6 * walktime) Footpos[i] = 0.06 + 2 * step;
		else if (time < 3.9 * walktime) Footpos[i] = 0.06 + 2 * step;
		else Footpos[i] = 0.06 + 2 * step;
	};
	return Footpos;
}
...
MatrixXd Trajectory::RF_zsimulation_leftwalk()
{
	...
	for (int i = 0; i < sim_n; i++)
	{
		...
		if (time < 1.05 * walktime) Footpos[i] = 0;
		else if (time < 1.25 * walktime) Footpos[i] = 0;
		else if (time < 1.45 * walktime) Footpos[i] = 0;
		else if (time < 2.05 * walktime) Footpos[i] = 0;
		else if (time < 2.25 * walktime) Footpos[i] = Step(time - 2.05 * walktime);
		else if (time < 2.45 * walktime) Footpos[i] = Stride(time - 2.05 * walktime);
		else if (time < 3.05 * walktime) Footpos[i] = 0;
		else if (time < 3.25 * walktime) Footpos[i] = Step(time - 3.05 * walktime);
		else if (time < 3.45 * walktime) Footpos[i] = Stride(time - 3.05 * walktime);
		else Footpos[i] = 0;
	};
	return Footpos;
};
...
MatrixXd Trajectory::LF_zsimulation_leftwalk()
{
	...
	for (int i = 0; i < sim_n; i++)
	{
		...
		if (time < 1.55 * walktime) Footpos[i] = 0;
		else if (time < 1.75 * walktime) Footpos[i] = Step(time - 1.55 * walktime);
		else if (time < 1.95 * walktime) Footpos[i] = Stride(time - 1.55 * walktime);
		else if (time < 2.55 * walktime) Footpos[i] = 0;
		else if (time < 2.75 * walktime) Footpos[i] = Step(time - 2.55 * walktime);
		else if (time < 2.95 * walktime) Footpos[i] = Stride(time - 2.55 * walktime);
		else if (time < 3.55 * walktime) Footpos[i] = 0;
		else if (time < 3.75 * walktime) Footpos[i] = 0;
		else if (time < 3.95 * walktime) Footpos[i] = 0;
		else Footpos[i] = 0;
	};
	return Footpos;
};
